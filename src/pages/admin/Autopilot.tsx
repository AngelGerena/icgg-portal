...
/**
 * Blog Schedule — the publishing calendar for Contra Corriente.
 *
    ...
    <>
      ...
      {scheduled.length === 0
        ? <div className="card"><div className="empty"><Icon name="clock" size={44} stroke={1.3} className="ei" />
            ...
            <p>{lang === 'es' ? 'Programa una entrada desde Contra Corriente.' : 'Schedule a post from Against the Current.'}</p></div></div>
      ...
      {rows.length === 0 && (
        <Empty icon="clock"
          ...
          sub={lang === 'es' ? 'Escribe una entrada en Contra Corriente y aparecerá aquí.' : 'Write a post in Against the Current and it will appear here.'} />
      )}
    </>
  );
}
